...
ATS_API ATS_HANDLES WINAPI Elapse(ATS_VEHICLESTATE vehicleState, int *panel, int *sound)
{
	...
	panel[231] = g_tims.Ammeter0; // 電流計[A]

	// panel[29] = g_sub.Key; // マスコンキー
	panel[30] = g_tims.AccelDelay; // 力行表示
	panel[35] = g_tims.BrakeDelay; // ブレーキ段
	panel[40] = g_tims.BrakeDelay == g_emgBrake ? 1 : 0; // 非常ブレーキ
	/* うさプラ併用
	panel[58] = (g_time / 3600000) % 24; // デジタル時
	panel[59] = g_time / 60000 % 60; // デジタル分
	panel[60] = g_time / 1000 % 60; // デジタル秒
	*//* うさプラ併用でオミット
	...
	*/
	...
	panel[197] = g_tims.Speed100; // 速度(100km/hの桁)
	panel[198] = g_tims.Speed10; // 速度(10km/hの桁)
	panel[199] = g_tims.Speed1; // 速度(1km/hの桁)
	panel[200] = g_tims.Speed; // 速度計
...
//ブレーキ圧ここから
	/*
	panel[87] = g_tims.BcPress0; // ブレーキシリンダ指針(0-180kPa)
	panel[88] = g_tims.BcPress1; // ブレーキシリンダ指針(200-380kPa)
	panel[89] = g_tims.BcPress2; // ブレーキシリンダ指針(400-580kPa)
	panel[90] = g_tims.BcPress3; // ブレーキシリンダ指針(600-780kPa)
	panel[97] = g_tims.MrPress0; // 元空気ダメ指針(750-795kPa)
	panel[98] = g_tims.MrPress1; // 元空気ダメ指針(800-845kPa)
	panel[99] = g_tims.MrPress2; // 元空気ダメ指針(850-895kPa)
	panel[100] = g_tims.BcCaution ? ((g_time % 1000) / 500) : 0; // 200kPa警告
	*/
	//追加分、デジタル表示の場合はこれを使用
	panel[87] = g_tims.BcPress4; // ブレーキシリンダ指針(100位)
	panel[88] = g_tims.BcPress5; // ブレーキシリンダ指針(10位)
	panel[89] = g_tims.BcPress6; // ブレーキシリンダ指針(1位)
	panel[90] = g_tims.BcPress7; // ブレーキシリンダ指針(int)
	panel[97] = g_tims.MrPress4; // 元空気ダメ指針(100位)
	panel[98] = g_tims.MrPress5; // 元空気ダメ指針(10位)
	panel[99] = g_tims.MrPress6; // 元空気ダメ指針(1位)
	panel[100] = g_tims.MrPress7; // 元空気ダメ指針(int)
	
//ブレーキ圧終り

//TIMS全般表示ここから
	panel[194] = g_tims.TimsSpeed100; // TIMS速度計(100km/hの桁)
	panel[195] = g_tims.TimsSpeed10; // TIMS速度計(10km/hの桁)
	panel[196] = g_tims.TimsSpeed1; // TIMS速度計(1km/hの桁)

	panel[13] = g_tims.Distance1000; // 走行距離(kmの桁)
	panel[14] = g_tims.Distance100; // 走行距離(100mの桁)
	panel[15] = g_tims.Distance10; // 走行距離(10mの桁)
	...
	panel[187] = g_tims.Number[0]; // 列車番号(千の桁)
	panel[188] = g_tims.Number[1]; // 列車番号(百の桁)
	panel[189] = g_tims.Number[2]; // 列車番号(十の桁)
	panel[190] = g_tims.Number[3]; // 列車番号(一の桁)
	panel[191] = g_tims.Charactor; // 列車番号(記号)

	panel[93] = g_tims.Number[3] != 0 ? 1 : 0; // 設定完了
	panel[94] = g_tims.PassMode; // 通過設定
	//うさプラ併用 panel[119] = g_tims.NextBlinkLamp; // 次駅停車表示灯
	panel[178] = g_tims.From; // 運行パターン発駅
	panel[179] = g_tims.Destination; // 運行パターン着駅
//TIMS全般表示終り

//TIMS電圧類ここから
	//うさプラ併用でオミット panel[217] = g_tims.AC; // 交流
	panel[221] = g_tims.DC; // 直流
	panel[207] = g_tims.CVacc; // 制御電圧異常
	panel[208] = g_tims.CVacc10; // 制御電圧(10位)
	panel[209] = g_tims.CVacc1; // 制御電圧(1位)
	/* うさプラ併用でオミット
	...
	*/
	panel[222] = g_tims.DCacc; // 直流電圧異常
	panel[223] = g_tims.DCacc1000; // 直流電圧(1000位)
	panel[224] = g_tims.DCacc100; // 直流電圧(100位)
	panel[225] = g_tims.DCacc10; // 直流電圧(10位)

	panel[233] = g_tims.Cvmeter; // 制御指針
	//うさプラ併用でオミット panel[231] = g_tims.Acmeter; // 交流指針
	panel[234] = g_tims.Dcmeter; // 直流指針
	
	panel[242] = g_tims.Accident; // 事故
	panel[243] = g_tims.Tp; // 三相
	panel[244] = g_tims.VCB; // VCB(DigitalNumber、0無表示～1入～2切)
	//うさプラ併用でオミット panel[235] = g_tims.alert_ACDC > 0 ? g_tims.alert_ACDC + ((g_time % 800) / 400) : 0; // 交直切替(点滅、無表示0、点滅1-2、3-4でペア)
//TIMS電圧類終り
...
//電列共通部分ここから
	...
	//うさプラ併用でオミット panel[135] = g_tims.HiddenLine[0] ? 0 : g_tims.Arrive[0][0]; // 到着時刻1H
	panel[215] = g_tims.HiddenLine[0] ? 0 : g_tims.Arrive[0][1]; // 到着時刻1M
	//うさプラ併用でオミット panel[137] = g_tims.HiddenLine[0] ? 0 : g_tims.Arrive[0][2]; // 到着時刻1S
	//うさプラ併用でオミット panel[138] = g_tims.HiddenLine[1] ? 0 : g_tims.Arrive[1][0]; // 到着時刻2H
	panel[216] = g_tims.HiddenLine[1] ? 0 : g_tims.Arrive[1][1]; // 到着時刻2M
	//うさプラ併用でオミット panel[140] = g_tims.HiddenLine[1] ? 0 : g_tims.Arrive[1][2]; // 到着時刻2S
	//うさプラ併用でオミット panel[141] = g_tims.HiddenLine[2] ? 0 : g_tims.Arrive[2][0]; // 到着時刻3H
	panel[217] = g_tims.HiddenLine[2] ? 0 : g_tims.Arrive[2][1]; // 到着時刻3M
	//うさプラ併用でオミット panel[143] = g_tims.HiddenLine[2] ? 0 : g_tims.Arrive[2][2]; // 到着時刻3S

	//うさプラ併用でオミット panel[144] = g_tims.HiddenLine[3] ? 0 : g_tims.Arrive[3][0]; // 到着時刻4H
	panel[218] = g_tims.HiddenLine[3] ? 0 : g_tims.Arrive[3][1]; // 到着時刻4M
	//うさプラ併用でオミット panel[146] = g_tims.HiddenLine[3] ? 0 : g_tims.Arrive[3][2]; // 到着時刻4S
	//うさプラ併用でオミット panel[147] = g_tims.HiddenLine[4] ? 0 : g_tims.Arrive[4][0]; // 到着時刻5H
	panel[219] = g_tims.HiddenLine[4] ? 0 : g_tims.Arrive[4][1]; // 到着時刻5M
	//うさプラ併用でオミット panel[149] = g_tims.HiddenLine[4] ? 0 : g_tims.Arrive[4][2]; // 到着時刻5S
...
//電列共通部分ここから
	/* うさプラ併用でオミット
	panel[215] = g_tims.HiddenLine[3] ? 0 : (g_tims.Next * g_tims.NextBlink); // 駅名表示(次駅、点滅する)
	panel[216] = g_tims.HiddenLine[3] ? 0 : g_tims.NextTime[0]; // 到着時刻(次駅、時)
	panel[217] = g_tims.HiddenLine[3] ? 0 : g_tims.NextTime[1]; // 到着時刻(次駅、分)
	panel[218] = g_tims.HiddenLine[3] ? 0 : g_tims.NextTime[2]; // 到着時刻(次駅、秒)
	panel[219] = g_tims.HiddenLine[3] ? 0 : g_tims.NextTrack; // 次駅到着番線
	*/
...
*/
	...
	sound[14+50] = g_spp.HaltChime3;
	sound[15+50] = g_spp.HaltChime;
	sound[16+50] = g_spp.PassAlarm;
	sound[17+50] = g_spp.HaltChime2;
